...
const FiltroComercios = () => {
  const [estado, setEstado] = useState("");
  const [planta, setPlanta] = useState("");
  const [metrosMin, setMetrosMin] = useState(5); // Actualizado
  const [precioMax, setPrecioMax] = useState(2000); // Actualizado
  ...
  const comerciosFiltrados = allComercios.filter((comercio) => {
    ...
    return (
      (estado === "" || 
        comercio.estado.toLowerCase() === estado.toLowerCase()) &&
      (planta === "" || comercio.planta === planta) &&
      comercio.metros >= metrosMin &&
      comercio.precio <= precioMax
    );
  });
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-elegant-gray-600 hidden sm:block">
                  Encuentra tu local comercial ideal
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <span className="hidden sm:inline">
                  {mostrarTodos ? "Aplicar filtros" : "Ver todos los Comercios"}
                </span>
              </button>
        ...
        {comerciosFiltrados.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center py-20">
            ...
            <p className="text-elegant-gray-600 max-w-md mb-6 leading-relaxed">
              Hemos buscado en todo nuestro catálogo de locales comerciales, pero no
          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b-2 border-gold-200">
              <div>
                <h3 className="text-xl font-semibold text-black">
                  {comerciosOrdenados.length}{" "}
                  {comerciosOrdenados.length === 1
                    ? "Local Comercial Encontrado"
                    : "Locales Comerciales Encontrados"}
                </h3>
            ...
            <div className="space-y-8">
              {comerciosOrdenados.map((comercio) => (
                <div key={comercio.id} className="animate-fadeIn">
                  <CardComercio data={comercio} />
                </div>
              ))}
            </div>
          </>
